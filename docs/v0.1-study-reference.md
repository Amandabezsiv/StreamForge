# StreamForge v0.1 Study Reference

> This document preserves the detailed requirements, domain model, experiment
> plan, engineering notes, and roadmap that originally lived in the main
> README. For the current project entry point, see [`README.md`](../README.md).
...
```text
Released baseline:  v0.1.0 — Local Distributed Baseline
Next phase:         v0.2.x — Orchestration and Elastic Scaling (planned)
Current runtime:    Docker Compose; Kubernetes is not implemented yet
```
...
## StreamForge v0.1.0 — Local Distributed Baseline (completed)

Version 0.1.0 established and measured the complete local architecture before
introducing an orchestrator. It remains the behavioral baseline that later
versions must preserve.
...
The main question answered by this version was:
...
The resulting baseline and planned direction are:

```text
v0.1.0
LOCAL DISTRIBUTED BASELINE
│
├── reliability ✅
├── PostgreSQL queue ✅
├── LISTEN/NOTIFY ✅
...
## StreamForge v0.2.x — Orchestration and Elastic Scaling (planned)

The first v0.2 milestone will run the existing application architecture on a
local Kubernetes cluster without intentionally changing its API, PostgreSQL job
queue, `LISTEN/NOTIFY`, lease recovery, or media-processing behavior.
...
The initial goal is orchestration parity, not redesign. Kubernetes manifests,
autoscaling, and pod-failure experiments will be introduced in later steps;
they are not implemented yet. Because v0.1 stores media on a local filesystem,
the Kubernetes design must provide storage shared by the API and workers before
multiple pods can preserve the same upload and output behavior.
...
- processing timing and capacity measurements;
- Prometheus, Grafana, and cAdvisor observability.

The following technologies remain outside the completed v0.1 baseline:
...
are documented in
[`ADR-002`](adr/ADR-002-prometheus-observability.md).
...
Prometheus, Grafana, and container metrics were completed in v0.1. Future
observability work may introduce:
...
This phase begins StreamForge v0.2.x. First reproduce the v0.1 runtime behavior
on a local Kubernetes cluster, then perform controlled failure and scaling
experiments.
...
After static Kubernetes deployments reproduce the v0.1 behavior, introduce HPA
in two measured stages:

```text
CPU-based HPA

queue-based HPA using a Prometheus/custom metric
```

CPU scaling is the simpler baseline. Queue-based scaling will then test whether
pending work is a better signal for this asynchronous workload. Kafka consumer
lag remains a future option only if the PostgreSQL queue is later replaced.
...
# Known Limitations at the v0.1 Boundary

The first version intentionally contains limitations.

Examples:

- PostgreSQL remains both the durable job queue and application database;
- `LISTEN/NOTIFY` accelerates pickup, but polling is still required as fallback;
- storage is local;
...
- worker output storage is not yet suitable for pods scheduled on different
  Kubernetes nodes.
